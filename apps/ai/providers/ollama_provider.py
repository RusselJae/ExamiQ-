"""Ollama-backed AI service implementations (cloud or local)."""
...
def _chat(
    ...
    return ollama_client.chat(prompt, system=system, max_output_tokens=max_output_tokens)


class OllamaCalibrationAnalyzer(CalibrationAnalyzer):
...
class OllamaCurriculumAdvisor(CurriculumAdvisor):
...
class OllamaDifficultyTagger(DifficultyTagger):
...
class OllamaQuestionGenerator(QuestionGenerator):
    def generate(
        ...
        for budget in token_budgets:
            result = ollama_client.chat_with_fallback(
                user_prompt,
                system=system,
            ...
            try:
                ...
                last_exc = exc
                logger.warning(
                    "Failed to parse Ollama question JSON (budget=%s): %s",
                    budget,
                    exc,
                )
...
class OllamaExplanationGenerator(ExplanationGenerator):
    def generate(self, question) -> dict:
        ...
        for budget in token_budgets:
            ...
            try:
                ...
                last_exc = exc
                logger.warning(
                    "Failed to parse Ollama explanation JSON (budget=%s): %s",
                    budget,
                    exc,
                )
...
class OllamaQuestionValidator(QuestionValidator):
