...
def navigation_context(request):
    ...
    if user.role == User.Role.STUDENT:
        from apps.analytics.concern_services import pending_concern_count_for_student
        from apps.reviews.models import ReviewSession

        context["pending_concern_count"] = pending_concern_count_for_student(user)
        context["chat_inbox_url"] = reverse("analytics_student:chat_inbox")
        context["show_chat_modal"] = False
    ...
    from apps.analytics.concern_services import pending_concern_count_for_professor

    context["pending_concern_count"] = pending_concern_count_for_professor(user)
    context["chat_inbox_url"] = reverse("analytics_professor:chat_inbox")
    context["chat_concerns_url"] = reverse("analytics_professor:chat_concerns_api")
    context["chat_note_url_base"] = reverse(
        "analytics_professor:chat_faculty_note", kwargs={"mistake_pk": 0}
    )
